(function ($, window) {
  ...
  const PollingManager = {
    ...
    _poll(pollingTemplate = "polling", isPolling = true) {
      $.ajax({
        url: `/wp-json/kkwoo/v1/payment-status`,
        method: "GET",
        ...
        success: (data) => {
          if (data.status === "completed" || data.status === "processing") {
            ...
            this._retry(isPolling, pollingTemplate, NORMAL_POLLING_INTERVAL);
          }
        },
        error: () => {
          this._retry(isPolling, pollingTemplate, MAX_POLLING_INTERVAL);
        },
      });
    },
