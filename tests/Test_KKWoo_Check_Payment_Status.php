...
class Test_KKWoo_Check_Payment_Status extends WP_UnitTestCase {
	...
	public function test_hooks_are_registered(): void {
		...
		$this->assertNotFalse(
			has_action( 'woocommerce_order_actions', array( $this->check_payment_status, 'custom_order_actions' ) ),
			'woocommerce_order_actions hook should be registered'
		);

		// Test woocommerce_order_action_check_payment_status_action hook
		$this->assertNotFalse(
			has_action( 'woocommerce_order_action_check_payment_status_action', array( $this->check_payment_status, 'check_payment_status_action' ) ),
		...
		$this->assertNotFalse(
			has_action( 'woocommerce_order_details_before_order_table', array( $this->check_payment_status, 'customer_check_payment_status_action' ) ),
	...
	public function test_custom_order_actions_empty_array(): void {
		$result = $this->check_payment_status->custom_order_actions( array() );
	...
	public function test_admin_notices(): void {
		$mock = $this->getMockBuilder( \KKWoo_Check_Payment_Status::class )
			->onlyMethods( array( 'is_admin_order_view_page' ) )
			->getMock();
	...
	public function paymentMethodProvider(): array {
		return array( array( 'kkwoo' ), array( 'paypal' ), array( 'stripe' ), array( 'cod' ), array( 'bacs' ) );
	}
}
