...
class Test_KKWoo_Payment_Gateway extends WP_UnitTestCase {
	...
	public function test_process_admin_options_hook_registered(): void {
		...
		$this->assertNotFalse(
			has_action( $hook, array( $this->gateway, 'process_admin_options' ) ),
	...
	public function test_after_settings_updated_hook_registered(): void {
		...
		$this->assertNotFalse(
			has_action( $hook, array( $this->gateway, 'after_settings_updated' ) ),
	...
	public function test_admin_missing_settings_notice_registered(): void {
		...
		$this->assertNotFalse(
			has_action( 'admin_notices', array( $this->gateway, 'admin_missing_settings_notice' ) ),
	...
	public function test_is_available( $enabled, $configured, $currency, $expected ): void {
		/** @var KKWoo_Payment_Gateway&\PHPUnit\Framework\MockObject\MockObject $gateway */
		$gateway = $this->getMockBuilder( KKWoo_Payment_Gateway::class )
			->onlyMethods( array( 'is_configured', 'wp_get_currency' ) )
			->getMock();
	...
	public function isAvailableDataProvider(): array {
		return array(
			'enabled, configured, KES'  => array( 'yes', true, 'KES', true ),
			'enabled, configured, USD'  => array( 'yes', true, 'USD', false ),
			'enabled, not configured'   => array( 'yes', false, 'KES', false ),
			'disabled, configured, KES' => array( 'no', true, 'KES', false ),
			'disabled, not configured'  => array( 'no', false, 'USD', false ),
		);
	}
}
