...
class Request_Validator
{
    ...
    */
    public static function validate_webhook_signature_presence(WP_REST_Request $request)
    {
        $signature = sanitize_text_field(
            wp_unslash($_SERVER['HTTP_X_KOPOKOPO_SIGNATURE'] ?? '')
        );
        if (empty($signature)) {
            return new WP_REST_Response(['status' => 'error', 'message' => 'Invalid request'], 401);
        }
    ...
    public static function validate_order_access(WP_REST_Request $request)
    {
        ...
        if (!$order_key) {
            return new WP_REST_Response(['status' => 'error', 'message' => 'Invalid order request'], 400);
        }
    ...
    */
    public static function validate_stk_push_callback(WP_REST_Request $request)
    {
        ...
        if (empty($payload['data'])) {
            return new WP_REST_Response(['status' => 'error', 'message' => 'Invalid request'], 400);
        }
