...
function kkwoo_handle_stk_push(WP_REST_Request $request)
{
    ...
    if (!$order_id) {
        return new WP_REST_Response(['status' => 'error', 'data' => 'Invalid order.'], 400);
    }

    $order = wc_get_order($order_id);
    if (!$order) {
        return new WP_REST_Response(['status' => 'error', 'data' => 'Order not found.'], 404);
    }

    if (!in_array($order->get_status(), ['pending', 'on-hold'], true)) {
        return new WP_REST_Response(['status' => 'error', 'data' => 'Invalid order status.'], 400);
    }

    require_once __DIR__ . '/stk-service.php';
    $response = kkwoo_send_stk_push($phone, $order_id);
...
function kkwoo_handle_stk_push_callback(WP_REST_Request $request)
{
    ...
    if ($status === 'Success' && isset($event['status']) && $event['status'] === 'Received') {
        ...
        $order->update_status('failed', 'Payment through Kopo Kopo for WooCommerce failed.');
        return new WP_REST_Response(['status' => 'error', 'message' => 'Payment failed'], 200);
    }
}
